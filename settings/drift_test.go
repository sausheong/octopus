...
func TestDocumentCarriesEveryConfigField(t *testing.T) {
	full := &config.Config{
		...
		Routing: config.RoutingCfg{
			...
			CostMode: config.CostModeAbsolute, CostReferenceUSD: 0.123,
			QualityFloors:    map[string]float64{"trivial": 0.61, "low": 0.71, "medium": 0.81, "high": 0.87},
			HighQualityFloor: 0.87, ReasoningBonus: 0.07,
